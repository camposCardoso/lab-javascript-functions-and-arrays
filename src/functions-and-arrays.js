...
function sum(arr) {
  ...
  let allZero = arr.every(item => item === 0);
  if (allZero) {
    return 0;
  }
...
function doesWordExist(arr, word) {
  // 1. Si el array está vacío, retornamos null
  if (arr.length === 0) {
    return null;
  }

  // 2. Si la palabra está en el array, retornamos true
  if (arr.includes(word)) {
    return true;
  }

  // 3. Si la palabra no está en el array, retornamos false
  return false;
}
...
function howManyTimes(arr, word) {
  // 1. Si el array está vacío, devolvemos 0
  if (arr.length === 0) {
    return 0;
  }

  // 2. Usamos filter para contar cuántas veces aparece la palabra
  const count = arr.filter(item => item === word).length;

  // 3. Retornamos el número de veces que aparece la palabra
  return count;
}
...
function greatestProduct(matrix) {
  // Ver si todos los números son 1
  const allOnes = matrix.every(row => row.every(num => num === 1));
  if (allOnes) return 1;

  // Ver si todos los números son 2
  const allTwos = matrix.every(row => row.every(num => num === 2));
